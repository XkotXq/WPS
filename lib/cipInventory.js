// Shared by anything that needs the CIP temporary-storage inventory -
// currently just app/dashboard/materials-list-cip/page.js. Kept in one
// place so the bypass fixture and the live CIP query only exist once.
...
// instead of a real CIP one - a real query would just fail against CIP
// with it, so callers substitute this fixed sample list instead, to keep
// pages usable while developing without CIP access.
export const BYPASS_TOKEN = "local-bypass";
...
async function queryCipInventory(token) {
...
export async function loadCipInventory(session) {
  if (session?.token === BYPASS_TOKEN) return { records: TEST_RECORDS, error: false };
  if (!session) return { records: [], error: true };
  try {
    const data = await queryCipInventory(session.token);
    return { records: data.records ?? [], error: false };
  } catch {
